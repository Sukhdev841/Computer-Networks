...
void* functional_service4(void* args)
{
  int output_fd = *((int*)(args));
      ...
      if(FD_ISSET(i,&read_fds))
      {
        ...
        if( i == mf_fd)
        {
          ...
          else
          {
            ...
            pthread_t id;
            cout<<"\nCreating thread for service.\n";
            int *args = new int;
            *args = clients[received_msg.client_name];
            switch(received_msg.service_type)
            {
              case 4:
                    // functional service1
                    pthread_create(&id,NULL,functional_service4,args);
                    break;
              default:
        ...
        else
        {
          ...
          int *args = new int;
          *args = clients[received_msg.client_name];
          cout<<"\nCreating thread for service "<<received_msg.service_type<<endl;
          switch(received_msg.service_type)
          {
            case 4:
                  // functional service1
